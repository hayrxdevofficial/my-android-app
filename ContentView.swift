...
struct ContentView: View {
    ...
    private var gameContent: some View {
        GameView(
            ...
            store: store,
            network: network,
            isMultiplayerMode: multiplayerGame,
            onSaveProgress: { s, c in
                if network.isAuthenticated {
                    network.submitScoreFinal(score: s, earnedCoins: c)
                }
            },
            onGameOver: { s, c in
    ...
    @ViewBuilder
    private var inviteAlertContent: some View {
        Button("Да") {
    ...
    // MARK: - Хелперы
    private func handleScreenChange(_ newScreen: AppScreen) {
        switch newScreen {
    ...
    private func handleGameStarted(_ started: Bool) {
...
struct AccountView: View {
    ...
    @ViewBuilder
    private func authFormContent(geo: GeometryProxy) -> some View {
        ScrollView {
            VStack(spacing: 14) {
...
struct GameView: View {
    ...
    @State private var liveSendTimer: Double = 0
    ...
    var isMultiplayer: Bool { isMultiplayerMode && network != nil }
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            ZStack {
                ...
                peerHeroLayer(heroSize: heroSize, x: peerHeroX)
                selfHeroLayer(heroSize: heroSize, x: myHeroX)
                hudLayer(geo: geo)
            }
    ...
    // MARK: - Слои
    @ViewBuilder
    private var enemiesLayer: some View {
    ...
    @ViewBuilder
    private func peerHeroLayer(heroSize: CGFloat, x: CGFloat) -> some View {
        if isMultiplayer, let hero = loader.hero {
            Image(uiImage: hero)
                .resizable().aspectRatio(contentMode: .fit)
                .frame(width: heroSize, height: heroSize)
                .hueRotation(.degrees(60))
                .position(x: x, y: remoteHeroY)
    ...
    func tick(w: CGFloat, h: CGFloat, heroSize: CGFloat,
        ...
        handleCollisions(myHeroX: myHeroX, heroSize: heroSize,
                         peerHeroX: peerHeroX, bulletSize: bulletSize)
    ...
    private func handleCollisions(myHeroX: CGFloat, heroSize: CGFloat,
                                  peerHeroX: CGFloat, bulletSize: CGFloat) {
    ...
    func triggerGameOver() {
        ...
        if let net = network, net.isAuthenticated {
            net.submitScoreFinal(score: score, earnedCoins: earnedCoins)
        }
